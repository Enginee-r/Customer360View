...
def generate_opportunities(accounts_df, n_per_account_avg=3):
    ...
    for _, account in accounts_df.iterrows():
        # FIRST: Ensure every account has at least 1-3 "Closed Won" opportunities for revenue
        # This guarantees all customers have some annual revenue and CLV
        n_won_opps = random.randint(1, 3)

        for i in range(n_won_opps):
            opp_id = f'OPP{random_string(15)}'
            stage = 'Closed Won'
            is_closed = True
            is_won = True

            # Distribute won opportunities across last 2 years (with more in current year)
            # 70% in current year, 30% in previous year
            if random.random() < 0.7:
                # Current year (2025)
                close_date = (datetime.now() - timedelta(days=random.randint(0, 365))).date()
            else:
                # Previous year (2024)
                close_date = (datetime.now() - timedelta(days=random.randint(365, 730))).date()

            opportunities.append({
                'opportunity_id': opp_id,
                'account_id': account['account_id'],
                'opportunity_name': f'{random.choice(product_lines)} Contract {i+1}',
                'deal_value': random.uniform(50000, 800000),  # Guaranteed revenue range
                'stage': stage,
        ...
        # SECOND: Add additional random opportunities (mix of open, lost, and more wins)
        n_additional_opps = random.randint(1, n_per_account_avg * 2)

        for i in range(n_additional_opps):
            opp_id = f'OPP{random_string(15)}'
